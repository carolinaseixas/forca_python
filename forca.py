...
def sortear_palavra():
    ...
    posicao_da_palavra = random.randint(0, len(palavras_possiveis)-1)
...
while (tentativas_restantes > 0) and (letras_restantes > 0):
    letra_escolhida = escolher_letra().lower()
    ...
    for posicao, letra in enumerate(palavra_sorteada):
        if letra == letra_escolhida:
            letras_restantes -= 1
            palavra_descoberta[posicao] = letra_escolhida
...
if letras_restantes == 0:
    print('\nParabéns! Você descobriu que a palavra secreta é {}!'.format(palavra_sorteada))
